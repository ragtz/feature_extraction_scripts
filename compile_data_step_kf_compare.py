...
#        list of objects to get volume features
...
def get_feature_vector(state,
    ...
    # position of objects
    for name, pos in objs.iteritems():
        if is_obj(name):
            for i in range(len(pos)):
                feature_names.append(name+'_position_'+str(i))
            features.extend(list(pos))
 
    # position of gripper
    for i in range(len(gripper_position)):
        feature_names.append('gripper_position_'+str(i))
    features.extend(list(gripper_position))
...
def extract_feature_time_series(bag_file, object_filters, hz=50):
    ...
    return fv_names, fv_ts, np.array(t)
...
def add_demo(bag_file, dataset, pid, task, demo_id, object_filters, steps):
    ...
    state_names, state, t = extract_feature_time_series(bag_file, object_filters) 

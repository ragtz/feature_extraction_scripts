...
def change(pre, post):
    ...
    return np.max(np.concatenate((var_pre_joint_ratio, var_post_joint_ratio)), axis=0)
...
def plot_change_values(c_values, fname, n_cols=4):
    ...
    for idx, name in enumerate(sorted(c_values.keys())):
        ...
        mean_0 = np.mean(c_values[name][0])
        mean_1 = np.mean(c_values[name][1])
        means = [mean_0, mean_1]

        std_0 = np.std(c_values[name][0])
        std_1 = np.std(c_values[name][1])
        stds = [std_0, std_1]

        i, j = idx/n_cols, idx%n_cols

        ax[i,j].set_title(name)
        ax[i,j].bar(labels, means, color='#297083', align='center')
        ax[i,j].errorbar(labels, means, stds, color='k', ls='none', lw=2, capthick=2)
